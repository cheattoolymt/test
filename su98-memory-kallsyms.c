...
//    unsigned long selinux_enabled = findSymbol(argv[0], search_base, "selinux_enabled");
    ...
    /* patch verity_map: mov w0,#0; ret */
    {
        unsigned long verity_map_addr = 0xffffffc000a4432cul;
        uint32_t nop_stub[2] = {
            0x52800000u, /* mov w0, #0 */
            0xD65F03C0u  /* ret        */
        };
        kernel_write(verity_map_addr, nop_stub, sizeof(nop_stub));
        message("MAIN: verity_map patched at 0x%lx", verity_map_addr);

        /* verify: read back and confirm */
        uint32_t verify[2] = {0, 0};
        kernel_read(verity_map_addr, verify, sizeof(verify));
        if (verify[0] == 0x52800000u && verify[1] == 0xD65F03C0u)
            message("MAIN: verity_map patch verified OK");
        else
            message("MAIN: **WARN** verity_map verify mismatch: %08x %08x", verify[0], verify[1]);

        /* remount /system rw */
        int rc = system("mount -o rw,remount /system");
        if (rc == 0)
            message("MAIN: /system remounted rw - OK");
        else
            message("MAIN: **WARN** remount returned %d", rc);
    }
